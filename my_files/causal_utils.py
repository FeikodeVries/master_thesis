...
class InstantaneousPrior(nn.Module):
    ...
    def forward(self, z_sample, target, action, z_mean=None, z_logstd=None, num_graph_samples=-1, z_shared=None,
        ...
        # latent_mask: shape [batch, num_z_samples, num_graph_samples, causal_vars, latent_vars (MLP for each latent),
        # latent_vars (input to NN)]
...
def mask_actions(actions, device, dropout_prob=0.1):
    # Mask actions
    ...
    # Set a value to be guaranteed to be taken. So there will never be a tensor without any actions
    ...
    if isinstance(actions, np.ndarray):
        ...
        fill = torch.randint(low=0, high=2, size=actions.shape).to(device=device)
