...
class PPO:
    def __init__(self, arglist):
        ...
        self.env = suite.load(domain_name=self.arglist.domain, task_name=self.arglist.task,
                              task_kwargs={'random': self.arglist.seed})
                ...
                if done or t % self.arglist.update_every == 0:
                    with torch.no_grad():
                        next_values = self.critic(
                            torch.tensor(o_1, dtype=torch.float64, device=self.device).unsqueeze(0))
                    values.append(next_values[0])
                    v_n_temp = []
                    gae_ = 0
                    gae_temp = []
                    for i in reversed(range(len(R))):
                        gae_ = self.arglist.gamma * self.arglist.gae_lambda * gae_ + R[i] + self.arglist.gamma * values[
                            i + 1] - values[i]
                ...
                if t % self.arglist.update_every == 0:
                    ...
                    for epoch in range(self.arglist.epochs):
                        ...
                        for i in range(0, O.size(0), self.arglist.batch_size):
                            ...
                            ratio = torch.exp(log_probs - torch.clamp(old_log_probs[indices], min=np.log(
                                1e-5)))  # clamp old_prob to 1e-5 to avoid inf
                            ...
                            surr2 = torch.clamp(ratio, 1 - self.arglist.ppo_clip_term,
                                                1 + self.arglist.ppo_clip_term) * gae_batch

                            actor_loss = -(
                                        torch.min(surr1, surr2) + self.arglist.entropy_weightage * dist.entropy().sum(
                                    1)).mean()
    ...
    def eval(self, episodes, render=False, save_video=False):
        ...
        if self.arglist.mode == "eval":
            ...
            if save_video:
                os.chdir(folder)
                subprocess.call(
                    ['ffmpeg', '-i', 'file%04d.png', '-r', '10', '-vf', 'pad=ceil(iw/2)*2:ceil(ih/2)*2', '-pix_fmt',
                     'yuv420p', 'video.mp4'])
...
def parse_args():
    ...
    # Common settings
    parser.add_argument("--domain", type=str, default="cartpole", help="cartpole / reacher")
    parser.add_argument("--task", type=str, default="swingup", help="swingup / hard")
