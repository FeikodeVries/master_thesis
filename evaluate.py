...
if __name__ == "__main__":
    ...
    run_name = f"{args.env_id}__evaluate__{args.seed}"
    ...
    # env setup
    envs = gym.vector.SyncVectorEnv([make_env(args.env_id, args.seed, args.capture_video, run_name, args.gamma,
                                              model=citris, batch_size=args.num_steps)])
    assert isinstance(envs.single_action_space, gym.spaces.Box), "only continuous action space is supported"

    agent = PPO_Agent(envs).to(device)
    path = str(pathlib.Path(__file__).parent.resolve()) + f'/runs/Walker2d-v4__ppo_continuous_action__1/ppo_continuous_action.cleanrl_model'
